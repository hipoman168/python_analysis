...
def recent_context(message_limit=24, meeting_limit=6):
    parts = []
    try:
        _, j = fetch_json(FAMILY_BUS + f"/messages?limit={message_limit}", headers=bridge_headers(), timeout=20)
        rows = j.get("messages") or []
        if rows:
            parts.append("最近對話：\n" + "\n".join(f"{m.get('sender_name') or m.get('sender_id')}: {m.get('content','')}" for m in rows[-message_limit:]))
    except Exception:
        pass
    try:
        _, j = fetch_json(FAMILY_BUS + f"/meetings?limit={meeting_limit}", headers=bridge_headers(), timeout=20)
        rows = j.get("meetings") or []
        if rows:
            parts.append("歷次會議摘要：\n" + "\n".join(
                f"- {m.get('topic','')}｜摘要：{m.get('summary') or '無'}｜決策：{json.dumps(m.get('decisions') or [], ensure_ascii=False)}｜未結：{json.dumps(m.get('open_items') or [], ensure_ascii=False)}"
                for m in reversed(rows)
            ))
    except Exception:
        pass
    try:
        _, j = fetch_json(FAMILY_BUS + "/open-actions", headers=bridge_headers(), timeout=20)
        rows = j.get("actions") or []
        if rows:
            parts.append("仍需追蹤工作：\n" + "\n".join(
                f"- [{a.get('status')}] {a.get('title')}｜owner={a.get('owner_agent_id') or '未指定'}｜{a.get('detail') or ''}"
                for a in rows[:30]
            ))
    except Exception:
        pass
...
def persist_meeting(session_id, topic, meeting_summary):
    payload = {
        ...
        "topic": topic,
        "summary": str((meeting_summary or {}).get("summary") or ""),
        "decisions": (meeting_summary or {}).get("decisions") or [],
    ...
    def do_GET(self):
        ...
        if parsed.path in ("/api/family/messages", "/api/family/meetings", "/api/family/open-actions"):
            try:
                ...
                suffix = {"/api/family/messages": "/messages", "/api/family/meetings": "/meetings", "/api/family/open-actions": "/open-actions"}[parsed.path]
                if suffix != "/open-actions":
                    limit = str(min(100, max(1, int((qs.get("limit") or ["40"])[0]))))
                    suffix += "?limit=" + limit
                _, j = fetch_json(FAMILY_BUS + suffix, headers=bridge_headers(), timeout=20)
        ...
        try:
            ...
            if self.path == "/api/discussion":
                ...
                for t in turns:
                    ...
                    if not reply:
                        continue
                    raw_id = str(t.get("agent") or "")
                    aid = "CEO-002" if raw_id == "CEO-002" else ("D2-004" if raw_id == "D2" else "D1-003")
                    receipts.append(family_post(aid, str(t.get("agent_name") or aid), reply, sid, message_type="DISCUSSION_TURN", metadata={"model": t.get("model"), "voice": t.get("voice"), "provider_role_fallback": t.get("provider_role_fallback", False), "dynamic": True}))
                summary = upstream.get("meeting_summary") or {}
                meeting_receipt = persist_meeting(sid, text, summary)
                if summary.get("summary"):
                    family_post("CEO-002", "002 陳啟航", str(summary.get("summary")), sid, message_type="MEETING_SUMMARY", metadata={"kind": "persistent_meeting_memory"})
                self.send_json(200, {"ok": True, "session_id": sid, "room_id": ROOM_ID, "mode": upstream.get("mode", "DYNAMIC_FAMILY_DISCUSSION"), "turns": turns, "meeting_summary": summary, "meeting_memory_receipt": meeting_receipt, "family_bus_receipts": receipts})
                return
            if self.path == "/api/conference":
                ...
                self.send_json(200, {"session_id": sid, "room_id": ROOM_ID, "mode": upstream.get("mode", "FAMILY_NAME_ROUTING"), "targets": upstream.get("targets", []), "turns": turns, "family_bus": receipts})
                return
            self.send_json(404, {"error": "not_found"})
